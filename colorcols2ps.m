function colorcols2ps(filename, cols1, color)
  p = mkcols2psparams();
  if size(cols1, 4) ~= 5;  
    
    cols1(:,:,:,5) = zeros(size(cols1));
    cols1(:,:,:,3) = cols1(:,:,:,1);
    cols1(:,:,:,1) = cols1(:,:,:,5);
  end
...
args(:,2:4) = vals2rgb(color(idx),jet);
args = sortrows(args);
args = args(:,2:end);
...
for kn = 1:nks
  ...
  for diri = 1:ncoldirs
    ...
    x = nonzeros(xs.*visible);
    y = nonzeros(ys.*visible);
    cosdir = cos(dir);
    sindir = sin(dir);
...
function output = bound(data, lower, upper)
